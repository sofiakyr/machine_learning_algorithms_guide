...
Similar to linear regression (part of the family of Generalized Linear Models). However it is part of a group of models
...
isNotFraud = 0). We are interested in finding how close a point is to $1$ (in other words its probability of success
(fraud in this case) plotted on the $y$ axis), as measured by the predictors ($x$, $z$ ... axis). The issue is that we
can get any value between $0$ and $1$, as well as values above and bellow that (e.g if the amount of money used in a
transaction is extremely large we might get something like $p = 2.3$ according to whatever coefficients in
...
of the odds on y-axis $log(\frac{P(x)}{1 - P(x)}) = \beta_0 + \beta_1x$, this makes things a lot easier (see more about
why it's helpful later on)
...
to minimize that value by moving the line. However the distance of the points from a random line (close to the real) in
...
difficult differentiation of products to a addition ($\log(a \times b) = \log(a) + \log(b)$), which is easy to solve.
...
requires all the data points as parameters to be able to make a model that perfectly fits them all and maximizes the
likelihood. If you are wondering why don't we just use that model, if it maximizes the likelihood, the answer is that it
...
overcomplications. The opposite of the saturated model is the Null model, a model that uses only one parameter (for
...
$1 - \frac\text{a measure of bad fit - a measure of fit of our model}}\text{a measure of bad fit - a measure of perfect fit}}$,
...
# Creates a general linear model (family = binomial specifies logistic regression)
...
library('DescTools')
# you will notice this is relatively low, it not easy to predict the survival of a passenger only given a few details about them
...
When we conduct such a study measuring how many smokers have developed lung dieses versus how many non smokers, and how
...
```R
# We can use a csv that is avalable online for our data, we can load this in R
...
# Review our model
summary(model)
# As expected BMI ang glu are significant (you can try and remove the predictors
# with a larger p-value but do this intrementally and note the effect it has on
...
#
# If predict a patient is diabettic when they are not that's a False Positive,

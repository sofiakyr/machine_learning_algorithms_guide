...
We say that the above function has been split in two _bais functions_ one for $x_j \geq c$ and one for $x_j < c$.

When we apply such a regression, we have manged to overcome some of the pitfalls of polynomial regression. However, the model whould be discontinious at the points of the knots, here is an example where we chosen a single knot at age 50. 
...
Smoothing splines is another way we can fit a curve to our data. Just like we did when fitting a line to the data, we will try to fit a curve that minimizes the residual error ( a curve that is the closest to all points). Of cource with a curve, we can always fit all the data perfectly, using potentially a very 'wiggly' curve that goes around all the points. Such a curve will have extreme problems of overfitting. Instead, what we really want to do is find a curve that fits the data well but also minimizes overfitting. 

We can achieve this in a similar manner to ridge regresssion, adding a penalty of weight $\lamdba$ to the function that minimizes RSS. This penatly will be assosiated with how 'wiggly' the line is, since we are trying to find the best fitting 'smooth' curve. We have explain that smoothness of the curve can be assosiated with the second derivative of the curve's function. This brings us to the function we will be minimizing (dont worry too much about the represantation):

$\sum_{n=1}^{n} (y_i - g(x_i)^2) + \lambda \int  g''(t)^2\,dt.$

As you whould expect, $\lambda$ is found using cross validation.
...
If all fails, there is the alternative of fitting a curve to your data using the local regression. This method creates the curve incrementaly. It chooses its place at some point $x$, by considering the location of the closets observations around that place. It resembles the algorithm k-nearest-neibours (discussed in future chapters).

Let's see exacly how this is achieved:
...
3. To find its new position we perform least squares on the k-closest points (including the original point). The original point is refered to us the vocal point. The closest those k points are, the more weight is given to them on the regression. They have more inflance on the position of the best fit line. The vocal point has the most weight.
...
6. We chould now draw a 'best fit curve' by connecting all those new points. However, that curve may have a lot of 'wiggle', so we want to follow some additional steps in order to smoothen it.

7. The 'wiggles' whould be the result of points that are too hight/low compared to the avarege, and are infuancing the position of the best fit line each time too much (remeber leverage points). TO reduce their effect, we add additional weights based on how far the original points are to the projected ones. Points that are moved too much will have higher weights.
...
* The process of regulization (smoothening) may need to be repeated a couple of times in order to reach the desired level of smoothness.

* We may want to fit parabolas insead of lines on each window of data. Parabolas tend to work better on data that indicates a lot of curving patterns. In R you can use the function lowess() for a line and loess() for a parabola (or a line, it defaults to parabola)

* In theory we can perform Local Regression for multiple $x$ predictors. However, when the dimentions are increasing the observations will become more sparse amongs them. This means that there whould be greater distances between them and not enought nearby observations for each dimention. You need to ensure you have enough observations. In generall local regresion is performed with 3 or 4 predictors as a max. 
...
#fitting parabollas
#spam associates with degree of smoothing and is revated with the window size selected proportional to the data
...
#fitting lines
#f is simila to span, the greater f gives more smoothnes
plot(Wage$age, Wage$wage, col="#CCBADC")

...
# gml() function withoud additional parameter will produce a linear model the same way lm() would
...
# on its own an error does really show us samething, it is valable when we compare it with errors from other models so that we can see which model is doing better.
...
# or if we want to combine various criteria, it may be useful to  visulise
par(mfrow = c(2, 2))
...
# We can see that probably something close to 10 would be optimal, let's see the selected 10 variables and their coefficients
coef(regfit.full,10)
...
# Another way to select between the best models from each pair (m0, m1, m2...mp),instead of using critira such us adjusted R-squared, would be cross validation. We will see how this could be done here. We choose to perform a 10-k fold cv.

# Let's set the seed again to ensure someone can repeat the test and get the same 'randomness'
set.seed(1)

# To perform 10-k fold cross validation, we first need to split the data in 10 folds (10 equal sized blockes).
k <- 10
# The way we split the data is we make a vector of size equal to nrow(Hitters) (the rows contained in the dataset Hitters), and each row will be assigned to a fold from 1 to 10 (sample 1:10). Since we want multibple rows to be part of the same fols we set replace=TRUE. Also because we haven't defined any paricular split ration, by default the split will be equal across the folds.
folds <- sample(1:k, nrow(Hitters), replace=TRUE)
#we can see that each row of the Hitters dataset has been assigned to one of the 10 k folds, in an equal manner
folds
...
for (i in 1:19) {
  # first we need to find ou the values for salary that each model would predict for our testing j fold
...
which.min(errors)
# let's see the suggested coeficients for lambda = 100
